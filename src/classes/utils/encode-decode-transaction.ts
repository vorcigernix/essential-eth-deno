...
export async function encodeData(
	...
	// adam: changed to use the Deno crypto library, original: const functionHash = hash.update(functionString).digest();
	const functionHash = new Uint8Array(
		await crypto.subtle.digest(
			"KECCAK-256",
			new TextEncoder().encode(functionString)
		)
	).toString();
