 ...
 */
export async function toChecksumAddress(address: string) {
	...
	const keccak = new Uint8Array(
		await crypto.subtle.digest("KECCAK-256", new TextEncoder().encode(_address))
	).toString();
	//adam: guess this is wrong as well
	const addressHash = keccak.replace(/^0x/i, "");
